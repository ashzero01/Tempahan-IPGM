...
<div class="container">
<h2>Tukar Gambar Baru {{ $vehicle->name }}</h2>
    ...
    <div class="form-group current-image">
        ...
        <div>
            <img src="{{ asset($vehicle->image ? $vehicle->image : 'default-vehicle.jpg') }}" alt="{{ $vehicle->name }}">
        </div>
    </div>
    ...
    <form action="{{ route('vehicles.update.image', $vehicle->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
